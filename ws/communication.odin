...
parse_response :: proc(
	...
	version, ok := version_parse(rline_str[:si])
	if !ok {
	...
	for {
		...
		key, ok := header_parse(&res.headers, line, allocator)
		if !ok {
	...
	if !headers_validate(&res.headers) {
		err = Request_Error.Invalid_Response_Header
		return
	}
...
request :: proc(
	...
	url, endpoint, parse_err := parse_endpoint(target)
	if parse_err != nil {
		return {}, Connection_Error.Invalid_URL
	}
	defer delete(url.queries)

	set_websocket_connection_headers(&request.headers, url.host)
...
_socket_stream_proc :: proc(
	...
	case .Read:
		...
		if recv_err == nil {
			...
			case .None:
				err = .None
			case .Not_Connected, .Connection_Closed, .Timeout:
...
_parse_body :: proc(
	...
	enc, has_enc := headers_get_unsafe(headers^, "transfer-encoding")
	length, has_length := headers_get_unsafe(headers^, "content-length")
	switch {
	case has_length:
	...
	if typ, ok := headers_get_unsafe(headers^, "content-type");
